...
public class ACSAlgorithmLocalInsert implements TravellingSalesmanSolver {
	...
	public ACSAlgorithmLocalInsert(final int[][] distance) {
	...
	private void localSearch() {
		for(Ant a : ants) {
			searchLocalImprovmentInsert(a);
		}
	}
	private void searchLocalImprovmentInsert(Ant a) {
		for(int i=1; i<nrOfNodes-1; i++) {
			Ant temp = a.copy();
			for(int j=i; j<nrOfNodes-1; j++) {
				int val = temp.getPath(j);
				temp.setPath(j, temp.getPath(j+1));
				temp.setPath(j+1, val);
				computePathLength(temp);
				if(temp.getPathLength() < a.getPathLength()) {
					a = temp;
					return;
				}
			}
